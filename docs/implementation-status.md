...
- реализована детерминированная demo-сессия с классическим входом по логину и паролю, 14 персонами и сохранением в `sessionStorage`; промежуточный MFA-маршрут удалён;
...
- `/auth/sign-in` — единая страница demo-авторизации по логину и паролю;
...
- прямой demo-вход по логину и паролю, выход, смена персоны;
...
- 28 августа 2026 карточка месторождения БГД дополнена компактным разделом «Кондиционные лимиты». Для каждого участка показаны действующий versioned-набор, дата, версия и состояние; «Полный список» открывает прокручиваемую таблицу «Параметр / Значение» с параметрами разреза, фильтров, бортового содержания, мощности, каротажа, запасов, плотности и поправок азимута. Данные используют существующий `ConditionSet`; старые IndexedDB-записи без списка лимитов получают совместимый набор при чтении. Evidence: [25-geobase-condition-limits-verification.md](./geology-functional-expansion/25-geobase-condition-limits-verification.md).
- 28 августа 2026 в карточке месторождения добавлено создание участка непосредственно из блока «Участки»: после заполнения неизменяемого кода и названия участок сохраняется и сразу становится доступен для кондиционных лимитов. При создании новой версии кондиций карточка участка показывает последнюю рабочую версию со статусом «Ожидает утверждения» и янтарным действием «Открыть и утвердить»; внутри списка отдельная зелёная кнопка «Утвердить версию» переводит черновик в утверждённое состояние. Опубликованная версия не скрывает ожидающую решения новую версию.
- 28 августа 2026 добавлен общий сброс локальных данных: в нижней части меню доступна команда «Сбросить демо данные» с пояснением о заполненных и изменённых значениях. Подтверждённая команда очищает IndexedDB и связанные in-memory данные, затем перезагружает систему с исходным набором. Та же команда доступна на аварийном экране, который перехватывает ошибку рендера приложения: пользователь может перезагрузить страницу или очистить локальные изменения без ручной работы с данными браузера.
- 4 сентября 2026 экран `/auth/sign-in` переработан в классическую форму «Логин / Пароль / Войти» без выбора профиля, SSO-перехода и MFA. В demo-режиме поля необязательны: submit сразу создаёт сессию и открывает рабочую область; прежний `/auth/mfa` удалён. Сгенерированный геологический фон зеркально отражён по горизонтали и заполняет весь viewport; desktop-композиция разделена 50/50, левый контент центрирован между верхним логотипом и нижней подписью, правая половина и форма получили полупрозрачные blur-слои. Hover кнопки использует стабильный градиент, световой блик, подъём и сдвиг стрелки без мерцания. Browser QA на 1440×900 и 390×844 подтвердил прямой переход на `/home`, отсутствие overflow и ошибок консоли; проходят `typecheck`, 47 test files / 118 tests, `lint` без errors (3 прежних warnings вне auth) и production `build`.
- 8 сентября 2026 `/geology/bgd` переработан под устойчивый продуктовый объём 2–4 месторождений: KPI-плашки, поиск, фильтры, сортируемая таблица, пагинация и связанные URL-параметры удалены. Экран показывает крупные карточки по две в ряд на desktop и по одной на mobile; текущий объект расположен первым, скрытые — последними. Карточка содержит код, RU/KZ/EN-названия, описание, количество участков и уникальных залежей, систему координат, версию, дату, выбор текущего объекта и переход к подробностям. Создание выделено отдельной двухстрочной CTA-кнопкой с устойчивым hover. Browser QA на 1280×720 и 390×844 подтвердил сетку 2→1, открытие формы, переход в карточку, отсутствие overflow и console errors. Проходят `typecheck`, 46 test files / 116 tests, `lint` без errors (3 прежних warnings вне БГД) и production `build`.
- 8 сентября 2026 подробная карточка месторождения `/geology/bgd/:depositId` разделена на расширяемые вкладки `Основные сведения`, `Участки и залежи`, `Кондиционные лимиты`, `Скважины`, `Редактирование` и permission-based `Аудит`. Активный раздел хранится в URL-параметре `section`, восстанавливается после reload и поддерживает прямые ссылки; панели остаются смонтированными, поэтому незавершённое редактирование не теряется. Реализованы семантика tablist/tab/tabpanel, счётчики, навигация ArrowLeft/ArrowRight/Home/End, mobile-прокрутка и автоматическое доведение активной вкладки в видимую область. Экскурсия получила отдельный шаг и открывает редактор через `?section=edit`. Browser QA на desktop и 390×844 подтвердил URL-навигацию, reload, клавиатуру, одну видимую панель, отсутствие page overflow и console errors; профиль без `geology.bgd.audit` не видит аудит, администратор открывает его по `?section=audit`. Проходят `typecheck`, 47 test files / 118 tests, `lint` без errors (3 прежних warnings вне БГД) и production `build` (остаётся общий chunk-size warning).
- 8 сентября 2026 в карточку скважины БГД добавлена URL-вкладка `Каротажи` (`?tab=logs`). Desktop-референс переосмыслен как responsive master-detail: слева компактный список исследований, справа дата, оператор, прибор, зона, источник, интервал/шаг, основной набор, комментарий и таблица каналов; ниже раскрывается встроенный просмотр кривых с глубинным курсором. Доступны сохраняемые добавление, редактирование, удаление, назначение основного набора и явно маркированный demo LAS-импорт; workspace, версии и audit сохраняются в IndexedDB. Добавлено разрешение `geology.bgd.well.manage-logs` для R1/R2/R13 и совместимые fallback-метаданные для прежних записей. Browser QA под R2 на desktop и 390×844 подтвердил прямой URL, редактирование с восстановлением v2 после reload, viewer, две исходные записи, auto-scroll активной вкладки и отсутствие page overflow. Проходят `typecheck`, 49 test files / 122 tests, `lint` без errors (3 прежних warnings вне среза) и production `build` (остаётся общий chunk-size warning).
- 9 сентября 2026 в карточку скважины БГД добавлены URL-вкладки `Керновые рейсы` (`?tab=core-runs`) и `Керновые пробы` (`?tab=core-samples`) по `UC.KAPGEO.BGD.08`. В минимальном master-detail интерфейсе список и редактор находятся на одном экране; длина рейса и процент выхода рассчитываются, перекрытия и невозможный выход блокируются. «Промер керна» вложен в выбранный рейс и поддерживает дату, оператора, колонку привязки, примечание и несколько интервалов с nullable мощностью дозы. Проба поддерживает несколько интервалов, привязку к рейсам, глубины по бурению/скорректированные глубины и лабораторные результаты с `<`/`>` и различием нуля/пустого значения. Общий versioned IndexedDB workspace переживает reload и пишет audit; удаление зависимого рейса блокируется, удаление пробы каскадно удаляет её дочерние данные после подтверждения. Права разделяют управление рейсами/пробами R1/R13 и промером R1/R2/R13; остальные профили read-only. Browser QA в Chrome подтвердил оба прямых URL в режиме `Минимум`, сохранение промера после reload, создание пробы и корректное отображение глубин/нулевого результата. Проходят `typecheck`, 51 test files / 126 tests, `lint` без errors (3 прежних warnings вне среза) и production `build` (остаётся общий chunk-size warning). Evidence: [26-bgd-core-data-verification.md](./geology-functional-expansion/26-bgd-core-data-verification.md).
- 15 сентября 2026 в карточку скважины БГД добавлена URL-вкладка `Литология` (`?tab=lithology`). Три раздельные колонки `По керну / По каротажу / Сводная` используют существующий versioned geology workspace; графическая колонка, таблица и редактор синхронизированы. Показаны границы, мощность, порода, минерализация, цвет, стратиграфия, описание и источник. Доступны add/split/merge/copy/delete, undo/redo, diff, validation, read-only и empty/error states; сохранение создаёт новую версию только изменённого track и audit event в IndexedDB. Старые локальные записи дополняются совместимо без общего reset. Browser QA под R2 подтвердил read-only, под R1 — переключение, редактирование и сохранение каротажного track как `v2`. Проходят `typecheck`, 53 test files / 129 tests, `lint` без errors (3 прежних warnings вне среза) и production `build`. Evidence: [27-bgd-lithology-verification.md](./geology-functional-expansion/27-bgd-lithology-verification.md).
- 15 сентября 2026 карточка скважины БГД дополнена URL-вкладкой `Рудные интервалы` (`?tab=ore-intervals`) по `UC.KAPGEO.BGD.10`. Добавлены обязательные `Источник выделения` и `Элемент`, прямой/дифференциальный режимы, таблицы рудных интервалов и ОРИ, формы CRUD, запрет пересечений внутри источника/элемента, автоматическое одиночное ОРИ, merge/split/detach и создание рудного интервала из непрерывных свободных дифференциальных интервалов. Workspace сохраняет version/relations/audit в IndexedDB; R1/R13 управляют всеми источниками, R2 — геофизическими, остальные видят read-only. Формулы содержания, метропроцента и агрегатов явно маркированы как demo до решения OQ-33; backend, production import и реальные данные не добавлялись. Browser QA подтвердил прямой URL, обязательные поля, дифференциальный режим, создание выделения из двух соседних интервалов, автоматическое четвёртое ОРИ, сохранение v2 после reload и отсутствие console errors. Проходят `typecheck`, 56 test files / 134 tests, `lint` без errors (3 прежних warnings вне среза) и production `build` (остаётся общий chunk-size warning). Evidence: [28-bgd-ore-intervals-verification.md](./geology-functional-expansion/28-bgd-ore-intervals-verification.md).
