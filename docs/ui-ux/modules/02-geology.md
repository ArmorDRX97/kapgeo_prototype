...
`/geology/bgd` реализует отдельный корневой реестр месторождений по пункту 1.4 GeoBase, а `/geology/bgd/:depositId` — самостоятельную карточку просмотра и редактирования. Для каждого объекта хранятся неизменяемые unique numeric ID/code, видимость, тип (`field`, `area`, `custom`), название, CRS, текстовое описание и первоначальный перечень участков/залежей. Создание открывает новую карточку; mutable update, hide/show и безопасное delete сохраняются в IndexedDB с version/audit. Удаление блокируется при внешних зависимостях. Поиск и фильтры передаются в карточку как URL-контекст и восстанавливаются при возврате.
