...
const tabs = [
  ...
  { id: 'lithology', label: 'Литология' },
  { id: 'ore-intervals', label: 'Рудные интервалы' },
] as const
...
function formFromWell(well: Well, depositId: string): CreateWellInput {
  ...
  const bgd = well.bgd
    ? { ...well.bgd, passport: { ...fallback.passport, ...well.bgd.passport } }
    : { ...fallback, name, depositId, profileId: well.profile === '—' ? '' : well.profile, geoBlockId: well.block === '—' ? '' : well.block, geometry: { ...fallback.geometry, headX: well.coordinates.x || null, headY: well.coordinates.y || null, acceptedDepth: well.depth || null }, drilling: { ...fallback.drilling, designDepth: well.depth || null, loggingDepth: well.depth || null }, statusHistory: [{ status: well.status, changedAt: fallback.statusChangedAt }] }
...
export function WellEditorPage({ depositId, wellId, activeTab: controlledActiveTab, onTabChange, onCancel, onSaved }: { depositId?: string; wellId?: string; activeTab?: BgdWellSection; onTabChange?: (tab: BgdWellSection) => void; onCancel: () => void; onSaved: (well: Well) => void }) {
  ...
  const canManageGeophysicalOreIntervals = hasPermission(persona, 'geology.bgd.well.manage-geophysical-ore-intervals')
...
function CoreRequiresSavedWell({ title }: { title: string }) {
  return <Panel title={title} description="Керновые данные связываются с сохранённой скважиной."><div className="geobase-empty"><Plus size={18} /><strong>Сначала создайте скважину</strong><span>После сохранения здесь появится работа с керновыми данными.</span></div></Panel>
}
...
function DescriptionTab({ form, master, currentDeposit, lenses, disabled, technologyDisabled, setFormField, setBgd, setGeometry, setPassport, calculateBottom }: { form: CreateWellInput; master: Awaited<ReturnType<typeof fetchGeologicalMasterData>> | undefined; currentDeposit: Awaited<ReturnType<typeof fetchGeologicalMasterData>>['deposits'][number] | undefined; lenses: Awaited<ReturnType<typeof fetchGeologicalMasterData>>['lenses']; disabled: boolean; technologyDisabled: boolean; setFormField: FormSetter; setBgd: BgdSetter; setGeometry: (key: keyof WellBgdData['geometry'], value: number | null) => void; setPassport: (key: keyof WellBgdData['passport'], value: string) => void; calculateBottom: () => void }) {
  ...
  return <div className="bgd-well-stack">
    ...
    <Panel title="Документация" description="Период составления документации по скважине."><div className="form-grid bgd-well-form-grid"><label className="field"><span className="field__label">Дата начала составления документации</span><input type="datetime-local" disabled={disabled} max={bgd.passport.documentEndDate || undefined} value={bgd.passport.documentStartDate} onChange={(event) => setPassport('documentStartDate', event.target.value)} /></label><label className="field"><span className="field__label">Дата окончания составления документации</span><input type="datetime-local" disabled={disabled} min={bgd.passport.documentStartDate || undefined} value={bgd.passport.documentEndDate} onChange={(event) => setPassport('documentEndDate', event.target.value)} /></label></div></Panel>
