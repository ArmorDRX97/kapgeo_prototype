...
const initialForm: CreateWellInput = {
  ...
  depositId: 'DEP-SARYTAU', siteId: 'SITE-NORTH', lensId: 'LENS-PR07', projectCode: 'PRJ-PR-07',
}
...
export function NewWellPage() {
  ...
  const validationError = useMemo(() => {
    if (step === 0) {
      ...
      if (!form.site || !form.profile || !form.depositId || !form.siteId || !form.lensId || !form.projectCode) return 'Укажите месторождение, участок, залежь и проект.'
    }
...
function IdentityStep({ form, setField }: { form: CreateWellInput; setField: <Key extends keyof CreateWellInput>(key: Key, value: CreateWellInput[Key]) => void }) {
  return <div className="wizard-section"><div className="wizard-section__intro"><RadioTower size={22} /><div><h2>Идентификация</h2><p>Код уникален в пределах предприятия и после публикации меняется через новую версию.</p></div></div><div className="form-grid"><label className="field"><span className="field__label">Код скважины <em>*</em></span><input value={form.code} onChange={(event) => setField('code', event.target.value.toUpperCase())} /><span className="field__hint">Формат WELL-0000</span></label><label className="field"><span className="field__label">Тип <em>*</em></span><select value={form.type} onChange={(event) => setField('type', event.target.value as WellType)}><option>Откачная</option><option>Закачная</option><option>Наблюдательная</option><option>Разведочная</option></select></label><label className="field"><span className="field__label">Назначение</span><select value={form.purpose} onChange={(event) => setField('purpose', event.target.value as WellPurpose)}><option>Эксплуатационная</option><option>Разведочная</option><option>Наблюдательная</option></select></label><label className="field"><span className="field__label">Участок</span><select value={form.site} onChange={(event) => setField('site', event.target.value)}><option>Северный</option><option>Центральный</option></select></label><label className="field"><span className="field__label">Профиль</span><input value={form.profile} onChange={(event) => setField('profile', event.target.value.toUpperCase())} /></label><label className="field"><span className="field__label">Месторождение</span><select value={form.depositId} onChange={(event) => setField('depositId', event.target.value)}><option value="DEP-SARYTAU">Сарытау</option></select></label><label className="field"><span className="field__label">Участок (контекст)</span><select value={form.siteId} onChange={(event) => setField('siteId', event.target.value)}><option value="SITE-NORTH">Северный</option><option value="SITE-CENTRAL">Центральный</option></select></label><label className="field"><span className="field__label">Залежь</span><select value={form.lensId} onChange={(event) => setField('lensId', event.target.value)}><option value="LENS-PR07">PR-07</option><option value="LENS-CN02">CN-02</option></select></label><label className="field"><span className="field__label">Проект</span><input value={form.projectCode} onChange={(event) => setField('projectCode', event.target.value.toUpperCase())} /></label></div></div>
}
...
function ReviewStep({ form }: { form: CreateWellInput }) {
  return <div className="wizard-section"><div className="wizard-section__intro"><Check size={22} /><div><h2>Проверка перед созданием</h2><p>Будет создана версия 1 в статусе «На проверке».</p></div></div><dl className="review-grid"><div><dt>Объект</dt><dd>{form.code}</dd><small>{form.type} · {form.purpose}</small></div><div><dt>Контекст</dt><dd>{form.site} · {form.profile}</dd><small>{form.depositId} · {form.lensId} · {form.projectCode}</small></div><div><dt>Координаты</dt><dd>{form.coordinates.x} / {form.coordinates.y}</dd><small>{form.crs}</small></div><div><dt>Конструкция</dt><dd>{form.depth} м · Ø {form.casingDiameter} мм</dd><small>Минимальный draft</small></div></dl><div className="form-alert"><CircleAlert size={17} /><span>После создания откроется единая карточка. Для публикации потребуются паспорт, контроль качества и согласование.</span></div></div>
}
