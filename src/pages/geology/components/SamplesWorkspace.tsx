...
import { labAnalyteQuantity, validateLabResults } from '../../../entities/well/lib/validateLabResults'
...
import { getAllowedUnits, getQuantityDefinition, toCanonicalValue } from '../../../shared/scientific/quantities/registry'
...
function LabResultsWorkspace({ well, sample, initial }: { well: Well; sample: Sample; initial: LabResult[] }) {
  ...
  return <Panel className="lab-workspace" title="Лабораторные результаты и QA/QC" description={`GEO-11 · ${sample.number}`} action={<Button size="sm" variant="secondary" onClick={add}><Plus size={15} /> Результат</Button>}>
    ...
    {results.length ? <div className="lab-results-table"><div className="lab-results-table__head"><span>Показатель</span><span>Значение / единица</span><span>Метод / аналитик</span><span>QA/QC</span></div>{results.map((result) => {
      const quantityId = labAnalyteQuantity(result.analyte)
      const canonical = toCanonicalValue({ quantityId, value: result.value, unitId: result.unit, qualifier: result.qualifier, missingReason: result.missingReason, source: { id: result.id, label: result.method } })
      const definition = getQuantityDefinition(quantityId)
      return <div key={result.id}><select aria-label={`Показатель ${result.id}`} value={result.analyte} onChange={(event) => update(result.id, { analyte: event.target.value as LabResult['analyte'], unit: event.target.value === 'pH' ? 'pH' : 'мг/кг' })}><option>U</option><option>Mo</option><option>pH</option></select><span className="lab-results-table__value"><select aria-label={`Квалификатор ${result.id}`} value={result.qualifier ?? ''} onChange={(event) => update(result.id, { qualifier: event.target.value ? event.target.value as ValueQualifier : undefined })}><option value="">=</option><option value="lt">&lt;</option><option value="lte">≤</option><option value="approx">≈</option><option value="gte">≥</option><option value="gt">&gt;</option></select><input aria-label={`Значение ${result.id}`} type="number" value={result.value ?? ''} onChange={(event) => update(result.id, { value: event.target.value === '' ? undefined : Number(event.target.value), missingReason: undefined })} /><select aria-label={`Единица ${result.id}`} value={result.unit} onChange={(event) => update(result.id, { unit: event.target.value as LabResult['unit'] })}>{getAllowedUnits(quantityId).map((unit) => <option key={unit.id}>{unit.id}</option>)}</select></span><span><strong>{result.method}</strong><small>{result.analyst}</small>{canonical !== undefined && <small>Канон.: {canonical.toLocaleString('ru-RU')} {definition.canonicalUnitId}</small>}</span><span><Badge tone={qaTone[result.qaStatus]}>{result.qaStatus}</Badge>{result.flag && <small>{result.flag}</small>}</span></div>
    })}</div> : <div className="empty-result"><ClipboardCheck size={24} /><strong>Результатов ещё нет</strong><span>Добавьте первый результат и отправьте его на QA/QC.</span></div>}
