...
import { GeologicalModelDataWorkspace } from './components/GeologicalModelDataWorkspace'
import { ConsumerHandoffBanner } from '../../features/geology-publication/ConsumerHandoffBanner'

export function ModelingPage() {
  const [created, setCreated] = useState(false)
  return <div className="page-stack"><ConsumerHandoffBanner consumer="modeling" /><GeologicalModelDataWorkspace /><PageHeader eyebrow="Модуль моделирования · MOD-01" title="Расчётные проекты" description="Проекты создаются из неизменяемых опубликованных snapshot GEO и TECH." actions={<button type="button" className="button button--primary button--md" onClick={() => setCreated(true)}><GitBranchPlus size={16} />Создать snapshot</button>} />
