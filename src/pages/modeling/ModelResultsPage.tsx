...
import { DgmDataWorkspace } from './components/DgmDataWorkspace'

export function ModelResultsPage() {
  const { projectId } = useParams({ from: '/modeling/results/$projectId' })
  const [published, setPublished] = useState(false)
  return <div className="page-stack"><DgmDataWorkspace /><PageHeader eyebrow="Модуль моделирования · MOD-16–21" title="RESULT-07" description="Карта и показатели по успешному запуску BASE-01." meta={<Badge tone={published ? 'success' : 'warning'} dot>{published ? 'Опубликовано' : 'Готово к публикации'}</Badge>} />
