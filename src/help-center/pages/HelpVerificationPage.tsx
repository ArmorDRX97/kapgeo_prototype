...
import { HelpBreadcrumbs, HelpFrame, HelpRouteLink } from '../ui/HelpFrame'
...
export function HelpVerificationPage() {
  return <HelpFrame eyebrow="Проверка руководства" title="Какие сценарии подтверждены" description="Реестр фактически открытых страниц и выполненных действий, на которых основана эта документация.">
    ...
    <div className="verification-list">{verificationRecords.map((record) => <article key={`${record.area}-${record.href}`}><CheckCircle2 size={18} /><div><h2>{record.area}</h2><p>{record.result}</p><small>Проверено: {record.checked}</small></div><HelpRouteLink href={record.href}>Открыть страницу →</HelpRouteLink></article>)}</div>
