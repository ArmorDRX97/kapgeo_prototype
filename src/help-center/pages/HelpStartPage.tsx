import { BookOpenCheck, KeyRound, MapPinned, Search } from 'lucide-react'
...
export function HelpStartPage() {
  return <HelpFrame eyebrow="Быстрый старт" title="С чего начать работу" description="Последовательность от входа до первого предметного действия. Маршрут занимает несколько минут и подходит любой роли.">
    ...
    <section className="onboarding-steps">
      <article><span>1</span><KeyRound size={22} /><div><h2>Войдите в систему</h2><p>В демонстрационном режиме поля можно оставить пустыми: кнопка «Войти» сразу откроет рабочую область доступного профиля.</p><GuideLink href="/auth/sign-in">Открыть страницу входа</GuideLink></div></article>
      <article><span>2</span><BookOpenCheck size={22} /><div><h2>Откройте «Мои задачи»</h2><p>Работу следует начинать с очереди: задача уже содержит объект, срок, модуль и ожидаемое действие.</p><GuideLink href="/work">Открыть задачи</GuideLink></div></article>
      <article><span>3</span><Search size={22} /><div><h2>Найдите объект</h2><p>Используйте глобальный поиск в шапке для WELL-1042, BLK-07-12, отчёта OP-DAY-03 или результата RESULT-07.</p><GuideLink href="/objects/wells/WELL-1042">Открыть WELL-1042</GuideLink></div></article>
      <article><span>4</span><MapPinned size={22} /><div><h2>Выполните ролевой сценарий</h2><p>Следуйте руководству своей роли. Каждый шаг содержит прямую ссылку и ожидаемый результат.</p><GuideLink href="/help/roles">Выбрать роль</GuideLink></div></article>
    </section>
