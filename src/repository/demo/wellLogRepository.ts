...
function seed(well: Well): WellLogWorkspace {
  const run: LogRunV2 = { id: `LOGV2-${well.id}-01`, name: 'Комплекс ГК + ПС + КС', source: 'LAS', from: 0, to: well.depth, step: .2, status: 'qc_issues', version: 1, curves: curves(), rawArtifactId: `RAW-LOG-${well.id}-01`, parserProfile: 'LAS 2.0', survey: { measuredAt: '2026-04-18', operator: 'Марат Омаров', instrument: 'РКС-3', zone: 'Ствол скважины', isPrimary: true, comment: 'Основной комплекс геофизических исследований. Требуется проверить разрыв 281,6–283,0 м и всплеск ГК на 418,2 м.' } }
  const controlRun: LogRunV2 = { id: `LOGV2-${well.id}-02`, name: 'Контрольный гамма-каротаж', source: 'DAT', from: 118, to: Math.max(118, well.depth - 4.4), step: .5, status: 'qc_passed', version: 1, curves: [curves()[0]!], rawArtifactId: `RAW-LOG-${well.id}-02`, parserProfile: 'DAT station', survey: { measuredAt: '2026-03-22', operator: 'Айдос Нурланов', instrument: 'СРП-68-03', zone: 'Продуктивный горизонт', isPrimary: false, comment: 'Контрольная запись после завершения бурения. Замечаний по качеству нет.' } }
  return { wellId: well.id, runs: [run, controlRun], layouts: [{ id: `LAYOUT-${well.id}-01`, name: 'Стандартный каротаж', trackOrder: ['GR', 'SP', 'RES'], widths: { GR: 1, SP: 1, RES: 1 }, visibleRange: { from: 240, to: 460 }, selectedDepth: 418.2, overlay: false, version: 1 }], activeLayoutId: `LAYOUT-${well.id}-01`, markers: [{ id: 'MARK-GAP-01', depth: 281.6, label: 'Gap 1,4 м', kind: 'gap' }, { id: 'MARK-SPIKE-01', depth: 418.2, label: 'GR spike', kind: 'spike' }], version: 1, updatedAt: '2026-08-24T00:00:00.000Z' }
}
export class DemoWellLogRepository {
  ...
  async applyImport(well: Well, current: WellLogWorkspace) {
    ...
    const run: LogRunV2 = { id: `LOGV2-${well.id}-${current.runs.length + 1}`, name: staging.fileName.endsWith('.DAT') ? 'Импортированный контрольный каротаж' : 'Импортированный комплекс ГИС', source: staging.fileName.endsWith('.DAT') ? 'DAT' : 'LAS', from: 0, to: well.depth, step: .2, status: staging.issues.some((issue) => issue.severity === 'error') ? 'qc_issues' : 'qc_passed', version: 1, curves: curves().map((curve) => ({ ...curve, id: `${curve.id}-I${current.runs.length + 1}` })), rawArtifactId: `RAW-${staging.id}`, parserProfile: staging.parserProfile, survey: { measuredAt: now().slice(0, 10), operator: 'Марат Омаров', instrument: 'Цифровая каротажная станция', zone: 'Ствол скважины', isPrimary: false, comment: `Демо-импорт ${staging.fileName}; ${staging.issues.length} замечания QC.` } }
